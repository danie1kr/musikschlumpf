...
static Schlumpf_Adafruit_VS1053_FilePlayer *myself;
...
boolean Schlumpf_Adafruit_VS1053_FilePlayer::useInterrupt(uint8_t type) {
  myself = this;  // oy vey
...
Schlumpf_Adafruit_VS1053_FilePlayer::Schlumpf_Adafruit_VS1053_FilePlayer(
...
boolean Schlumpf_Adafruit_VS1053_FilePlayer::begin(void) {
...
/*
boolean Schlumpf_Adafruit_VS1053_FilePlayer::playFullFile(const char *trackname) {
...
void Schlumpf_Adafruit_VS1053_FilePlayer::stopPlaying(void) {
  // cancel all playback
...
void Schlumpf_Adafruit_VS1053_FilePlayer::pausePlaying(boolean pause) {
  if (pause) 
    playingMusic = false;
  else {
    playingMusic = true;
...
boolean Schlumpf_Adafruit_VS1053_FilePlayer::paused(void) {
  return (!playingMusic && currentTrack);
}

boolean Schlumpf_Adafruit_VS1053_FilePlayer::stopped(void) {
  return (!playingMusic && !currentTrack);
}

// Just checks to see if the name ends in ".mp3"
boolean Schlumpf_Adafruit_VS1053_FilePlayer::isMP3File(const char* fileName) {
  return (strlen(fileName) > 4) && !strcasecmp(fileName + strlen(fileName) - 4, ".mp3");
}

unsigned long Schlumpf_Adafruit_VS1053_FilePlayer::mp3_ID3Jumper(File mp3) {
...
boolean Schlumpf_Adafruit_VS1053_FilePlayer::startPlayingFile(SdFat &fat, const char *trackname) {
  // reset playback
  ...
  currentTrack = fat.open(trackname);
  if (!currentTrack) {
    return false;
  }
...
void Schlumpf_Adafruit_VS1053_FilePlayer::feedBuffer(void) {
  noInterrupts();
...
void Schlumpf_Adafruit_VS1053_FilePlayer::feedBuffer_noLock(void) {
